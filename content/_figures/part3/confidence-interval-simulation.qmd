...
leftPlot = Plot.plot({
  width: Math.min(680, Math.max(300, width/2 - 20)),
  height: 340,
  ...
  return html`<div class="ojs-toolbar-grid">
    <div class="left">${viewof resample}</div>
      <div class="center" role="status" aria-live="polite">
        ...
        This interval **${currentCoveredZ ? "covers" : "does not cover"}** the fixed true value ${tex`\mu=${controls.trueMu.toFixed(1)}`}.`}
      </div>
...
html`<div class="ci-row">
  <div class="ci-panel">${leftPlot}</div>
  <div class="ci-panel">${rightPlot}</div>
</div>`
...
@media (max-width: 760px) {
  .ci-row { flex-direction: column; overflow-x: visible; }
  .ci-panel { flex: 1 1 auto; min-width: 0; width: 100%; }
  .controls-grid { grid-template-columns: 1fr; }
  .controls-col { min-width: 0; }
  .ojs-toolbar-grid { grid-template-columns: 1fr; gap: 8px; }
  .ojs-toolbar-grid .left form,
  .ojs-toolbar-grid .right form { justify-content: center !important; }
}
</style>`
