...
### Summary of Conjugate Priors and Posterior Updates
...
| $X_i \mid \lambda \sim \mathrm{Poisson}(\lambda)$ (i.i.d.) | $\lambda \sim \mathrm{Gamma}(a,b)$ | $\lambda \mid \underline{x} \sim \mathrm{Gamma}(a + n\bar{x},\; b + n)$ | <span class="xref-no-num">[Link @q-gamma-pois]</span> |
| $X_i \mid p \sim \mathrm{Geom}(p)$ (i.i.d.) | $p \sim \mathrm{Beta}(a,b)$ | $p \mid \underline{x} \sim \mathrm{Beta}(a + n,\; b + n(\bar{x}-1))$ | <span class="xref-no-num">[Link @q-beta-geom]</span> |
| $X_i \mid \mu \sim \mathcal{N}(\mu,\sigma^2)$ (known $\sigma^2$) | $\mu \sim \mathcal{N}(\mu_0,\sigma_0^2)$ | $\mu \mid \underline{x} \sim \mathcal{N}(M,V)$ |  <span class="xref-no-num">[Link @q-normal-normal]</span> |
| $X_i \mid \gamma \sim \mathcal{N}(\mu,\gamma)$ (known $\mu$) | $\gamma \sim \operatorname{InvGamma}(a,b)$ | $\gamma \mid \underline{x} \sim \mathrm{InvGamma}\!\left(a + n/2,\; b + \tfrac12 \sum (x_i - \mu)^2\right)$ | <span class="xref-no-num">[Link @q-normal-invgamma]</span> |
| $X_i \mid \lambda \sim \mathrm{Exponential}(\lambda)$ (i.i.d.) | $\lambda \sim \mathrm{Gamma}(a,b)$ | $\lambda \mid \underline{x} \sim \mathrm{Gamma}\left(a + n,\; b + n\bar{x}  \right)$ | <span class="xref-no-num">[Link @q-exp-gamma]</span> |
