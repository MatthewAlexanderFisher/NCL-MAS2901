...
$$
for some constant $k \in \mathbb{R}$ that does **not** depend on $\theta$.
...
|------------------|--------|------------|-------|
| $X \mid p \sim \;  \mathrm{Binomial}(n,p)$ | $p \sim \mathrm{Beta}(a,b)$ | $p \mid x \sim \mathrm{Beta}(a + x,\; b + n - x)$ | <span class="xref-no-num">[Link @q-beta-binom]</span> |
| $X_i \mid \lambda \; \overset{\text{i.i.d.}}{\sim} \;  \mathrm{Poisson}(\lambda)$  | $\lambda \sim \mathrm{Gamma}(a,b)$ | $\lambda \mid \underline{x} \sim \mathrm{Gamma}(a + n\bar{x},\; b + n)$ | <span class="xref-no-num">[Link @q-gamma-pois]</span> |
| $X_i \mid p \; \overset{\text{i.i.d.}}{\sim} \;  \mathrm{Geom}(p)$  | $p \sim \mathrm{Beta}(a,b)$ | $p \mid \underline{x} \sim \mathrm{Beta}(a + n,\; b + n(\bar{x}-1))$ | <span class="xref-no-num">[Link @q-beta-geom]</span> |
| $X_i \mid \mu \; \overset{\text{i.i.d.}}{\sim} \;  \mathcal{N}(\mu,\sigma^2)$ (known $\sigma^2$) | $\mu \sim \mathcal{N}(\mu_0,\sigma_0^2)$ | $\mu \mid \underline{x} \sim \mathcal{N}(M,V)$ |  <span class="xref-no-num">[Link @q-normal-normal]</span> |
| $X \mid \gamma \; \overset{\text{i.i.d.}}{\sim} \; \mathcal{N}(\mu,\gamma)$ (known $\mu$) | $\gamma \sim \operatorname{InvGamma}(a,b)$ | $\gamma \mid \underline{x} \sim \mathrm{InvGamma}\!\left(a + n/2,\; b + \tfrac12 \sum (x_i - \mu)^2\right)$ | <span class="xref-no-num">[Link @q-normal-invgamma]</span> |
| $X_i \mid \lambda \; \overset{\text{i.i.d.}}{\sim} \;  \mathrm{Exponential}(\lambda)$  | $\lambda \sim \mathrm{Gamma}(a,b)$ | $\lambda \mid \underline{x} \sim \mathrm{Gamma}\left(a + n,\; b + n\bar{x}  \right)$ | <span class="xref-no-num">[Link @q-exp-gamma]</span> |
| $X_i \mid \theta \; \overset{\text{i.i.d.}}{\sim} \;  \mathrm{Weibull}(\beta, \theta)$ (known $\beta$) | $\theta \sim \mathrm{InvGamma}(a,b)$ | $\theta \mid \underline{x} \sim \mathrm{InvGamma}\left(a + n,\; b + \sum_{i=1}^n x_i^\beta  \right)$ | <span class="xref-no-num">[Link @q-weib-invgamma]</span> |
...
In Bayesian inference, **estimators** are obtained by summarising the **posterior distribution**
$$
\pi(\theta \mid \underline{x})
= \frac{f(\underline{x}\mid\theta)\, \pi(\theta)}{f(\underline{x})}.
$$

We often choose a single value to represent our updated belief about the parameter.  
Different summaries of the posterior lead to different Bayesian estimators.  
...
3. **Differentiate and solve:**  
   We differentiate with respect to $\theta$ and set the derivative equal to zero:
   $$
   \frac{\partial}{\partial\theta}\,\log \pi(\theta \mid \underline{x}) = 0.
   $$
   Solving this equation yields the MAP estimator:
   $$
   \hat{\theta}_{\mathrm{MAP}}
   = \arg\max_{\theta}\, \pi(\theta \mid \underline{x}).
   $$
...
- take logs,  
- differentiate,  
- solve for the maximiser.

This is **identical** to the MLE procedure — except the prior contributes an extra term.
