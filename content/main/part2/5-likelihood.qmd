...
- If the data are **not identically distributed**, only **Approach 1** applies.  
...
### Asymptotic Variance of an Efficent Estimator
...
Let $\hat{\theta}$ denote the <span class="xref-no-num">[maximum likelihood estimator @def-mle]</span> based on $n$ i.i.d. samples $X_1, \ldots, X_n$ drawn from a distribution with density or mass function $f(x \mid \theta)$.

The **observed Fisher information** is the <span class="xref-no-num">[Fisher information @def-fisher-information]</span> evaluated at the **observed value** of the MLE, $\hat{\theta}_{\text{obs}}$, obtained from the observed data $x_1, \ldots, x_n$:
$$
