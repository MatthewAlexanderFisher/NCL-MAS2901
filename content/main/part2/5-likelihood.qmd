...
$$
  V_i \sim \mathrm{Pois}(\alpha)
$$

Derive the **likelihood function**.
:::

::: {#sol-poisson-likelihood}
...
$$
\mathcal{I}(\theta) \;=\; -\,\mathrm{E}\!\left[ \frac{\partial^2}{\partial \theta^2}\,\ell(\theta;\underline{X}) \right].
$$
:::

**Intuition:** Fisher information measures how much information the data carry about $\theta$.

* If the log-likelihood is sharply peaked around its maximum, a small change in $\theta$ makes the likelihood much worse — so the data are highly informative.
* If the log-likelihood is flat, many values of $\theta$ fit almost equally well — so there is little information.
  The “peakedness” is captured by the curvature (second derivative) of the log-likelihood
...
The quantity $\tfrac{1}{\mathcal{I}(\theta)}$ is called the **Cramér–Rao Lower Bound ([CRLB]{title="Cramér–Rao Lower Bound"})**. It represents the *smallest possible variance* achievable by any unbiased estimator of $\theta$.
:::
...
**Step 3 (Proving $\mathrm{Var}(U) = \mathcal{I}(\theta)$):**
...
&= \mathrm{E}\left[\frac{\frac{\partial^2}{\partial \theta^2} f(\underline{X}\mid \theta)}{f(\underline{X}\mid \theta)}\right] + \underbrace{\mathrm{E}\left[-\frac{\partial^2}{\partial \theta^2} \ell(\theta ; \underline{X})\right]}_{= \mathcal{I}(\theta)}.
\end{align*}
...
$$
\mathrm{Var}(\hat{\theta}) \;\geq\; \frac{\mathrm{Cov}(\hat{\theta},U)^2}{\mathrm{Var}(U)} \;=\; \frac{1^2}{\mathcal{I}(\theta)} \;=\; \frac{1}{\mathcal{I}(\theta)}.
$$
...
* An unbiased estimator is called **efficient** if its variance *achieves* the [CRLB]{title="Cramér–Rao Lower Bound"} exactly:
  ...
  $$
  \mathrm{Var}(\hat{\theta}) = \frac{1}{\mathcal{I}(\theta)}.
  $$
* A sequence of estimators $\hat{\theta}_n$ is **asymptotically efficient** if, as the sample size grows, its variance approaches the [CRLB]{title="Cramér–Rao Lower Bound"}:
  $$
  \lim_{n \to \infty} \frac{\mathrm{Var}(\hat{\theta}_n)}{1/\mathcal{I}(\theta)} \;=\; 1,
  $$
  where $\mathcal{I}(\theta)$ is the <span class="xref-no-num">[Fisher information @def-fisher-information]</span>
