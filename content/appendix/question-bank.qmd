...
The bank is deliberately broad: it is a place to **practise**, not a list of
questions that should all be completed from top to bottom. Try a question before
opening its solution, mark the step at which you became stuck, and then retry it
without the solution a day or two later.
...
|---|---|---|
| checking the foundations | `probability`, `expectation`, `estimator` at **Easy** or **Medium** | `bias`, `variance`, `mse` |
| revising estimation | `likelihood` and `mle` at **Medium** | `fisher-information`, `consistency`, `asymptotic-mle` |
| revising uncertainty and tests | `confidence-interval` and `hypothesis-test` at **Medium** | `t-test`, `lrt`, then `exam-style` |
| revising Bayesian methods | `bayes` and `conjugacy` at **Medium** | `map`, `hdi`, then `exam-style` |

Use **Challenge** questions for extension or after you can complete the core
route unaided. Some questions include coding because it can help you explore the
ideas; writing or interpreting code is not required in the exam.
:::

The filters below combine tags using **AND**: selecting `bayes` and `binomial`
shows questions carrying both tags. Difficulty runs from **Easy** through
**Challenge**.
...
```{ojs}
{
  for (const el of allQuestions) {
    ...
    if (header && !header.querySelector('.difficulty-peppers')) {
      ...
      const difficultyName = ["Easy", "Medium", "Hard", "Challenge"][level] ?? "Unrated";
      span.setAttribute('aria-label', `Difficulty: ${difficultyName}`);
...
viewof levels = Inputs.checkbox(
  [0, 1, 2, 3],
  {
    ...
    format: d => ["Easy", "Medium", "Hard", "Challenge"][d]
  }
)
...
\hat\mu_{\mathrm{obs}} = \frac{1}{n}\sum_{i=1}^n x_i = \bar x.
$$

So the MLE is the sample-mean estimator $\bar X$; for these observed data its
value is $\bar x$.
