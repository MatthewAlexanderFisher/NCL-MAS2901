...
### Binomial {#sec-binom}
**Model.** $X \sim \operatorname{Binomial}(n,p)$, $n\in \mathbb{N}$, $0<p<1$.  
...
$$
**Statistics.** $\operatorname{E}[X]=\frac{b}{a-1},\quad \operatorname{Var}(X)=\frac{b^2}{(a-1)^2(a-2)}$ and $\text{Mode}(X) = \frac{b}{a+1}$ if $a > 2$.
...
$$
**Mean/Cov.** $\operatorname{E}[\mathbf{X}]=\boldsymbol{\mu},\quad \operatorname{Var}(\mathbf{X})=\Sigma.$
