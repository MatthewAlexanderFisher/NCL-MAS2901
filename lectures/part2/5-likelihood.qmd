...
In the following, we use $\mathrm{Distribution}(\theta)$ as a **placeholder** to indicate “a distribution depending on parameter $\theta$.”
...
This allows us to keep definitions *general*! 
:::
...
$$
X_i \mid \theta \overset{\text{iid}}{\sim} \mathrm{Distribution}(\theta),
\quad i = 1, \ldots, n.
$$
...
$$
X_i \mid \theta \sim \mathrm{Distribution}_i(\theta),
$$
...
$$
X_{t+1} \mid X_t = x_t, \, \sigma^2 \sim \mathcal{N}(x_t, \sigma^2).
$$
...
| **Independent (not identical) data** | Independent; each $X_i$ may have its own density $f_i(x_i\mid\theta)$ | $f(\underline{x}\mid\theta) = \prod_{i=1}^n f_i(x_i\mid\theta)$ |
...
## Estimator Properties of the MLE
